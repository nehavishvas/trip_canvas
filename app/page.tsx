    ...
    <div className="flex flex-col min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50 transition-colors duration-300">
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8 space-y-16 relative z-10">
        ...
        {!hasActiveFilters && featuredBlog && (
          <section className="relative overflow-hidden rounded-3xl border border-zinc-200 bg-white p-6 sm:p-8 lg:p-10 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40 text-zinc-900 dark:text-white group">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center relative z-10">
              ...
              <div className="lg:col-span-7 flex flex-col justify-center space-y-5">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-md bg-amber-50 border border-amber-200 px-2.5 py-1 text-[10px] font-bold tracking-wider text-amber-800 uppercase dark:bg-amber-950/20 dark:border-amber-900/50 dark:text-amber-400">
                    Featured
                  </span>
                  {featuredBlog.category && (
                    <span className="rounded bg-zinc-100 dark:bg-zinc-800 px-2.5 py-0.5 text-[10px] font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">
                      {featuredBlog.category.name}
                    </span>
                  )}
                </div>
                
                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold tracking-tight leading-[1.2] text-zinc-900 dark:text-white group-hover:text-amber-500 dark:group-hover:text-amber-400 transition-colors">
                  <Link href={`/blog/${featuredBlog.slug}`}>
                    {featuredBlog.title}
                  </Link>
                </h2>
                
                <p className="text-zinc-500 dark:text-zinc-400 text-xs sm:text-sm leading-relaxed line-clamp-3 max-w-xl">
                  {featuredBlog.content.replace(/[#*`_]/g, '')}
                </p>
                
                {/* Author & Meta */}
                <div className="flex flex-wrap items-center gap-4 pt-5 border-t border-zinc-100 dark:border-zinc-800/60 text-xs text-zinc-400">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-semibold text-zinc-700 dark:text-zinc-300">{featuredBlog.author?.name}</p>
                    </div>
                  </div>
                  ...
                  <Link
                    ...
                    className="sm:ml-auto inline-flex items-center gap-1 rounded-xl bg-zinc-950 text-white px-5 py-2.5 text-xs font-semibold shadow hover:bg-amber-500 hover:text-white dark:bg-white dark:text-zinc-950 dark:hover:bg-amber-500 dark:hover:text-white transition-all duration-200"
                  >
                    Read Journal
            ...
            <div className="sticky top-24 space-y-8">
                ...
                <div className="flex flex-col gap-2">
                  {db.users.filter(u => u.role === 'admin' || u.role === 'author').map(author => {
                    ...
                    return (
                      ...
                      >
                        <div className={`flex h-7 w-7 items-center justify-center rounded-full text-[10px] font-bold ${isActive ? 'bg-zinc-800 text-white dark:bg-zinc-200 dark:text-zinc-950' : 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400'}`}>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900/40">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 text-center">
                  <div className="rounded-xl bg-zinc-50 p-3.5 border border-zinc-100 dark:bg-zinc-900/50 dark:border-zinc-800">
